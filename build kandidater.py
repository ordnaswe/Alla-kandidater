...
def build_data(csv_text, geo, old_data):
    ...
    for row in rdr:
        ...
        if grp is None:
            grp = {"vt": vt_bit, "area": g(row, "VALKRETSNAMN"), "fk": fk,
                   ...
                   "omr_kod2": g(row, "VALOMRÅDESKOD").zfill(2)[:2] if g(row, "VALOMRÅDESKOD") else "",
                   "omr_namn": g(row, "VALOMRÅDESNAMN")}
            groups[gkey] = grp
    ...
    # ---- BALLOT: bläddra kommun -> {kommun/region/riksdag} -> parti -> kandidater ----
    # Struktur:
    #   lan:    { länidx: [ [kommunkod, kommunnamn], ... ] }        (för län->kommun-menyn)
    #   kf:     { kommunkod: { fk: [personindex...] } }             (kommunfullmäktige)
    #   rf:     { regionnyckel: { fk: [personindex...] } }          (region, delad per region)
    #   rd:     { valkretsnamn: { fk: [personindex...] } }          (riksdag, delad per valkrets)
    #   komMap: { kommunkod: [regionnyckel, rdvalkrets, kommunnamn, länidx] }
    # Namn hämtas ur srec via personindex -> inga namn dubbleras.
    # rvk (riksdagsvalkrets per kommun) och regiontillhörighet läses ur GEO-blocket.
    geo_rvk = {}      # kommunkod -> riksdagsvalkrets-namn
    for gf in (geo if isinstance(geo, list) else geo.get("features", [])):
        if isinstance(gf, dict) and gf.get("c"):
            geo_rvk[gf["c"]] = gf.get("rvk", "")
    ...
    def grp_indices(grp):
        ...
        for knum, ordn in mem:
            ...
            seen.add(pi); out.append(pi)
        return out

    bal_kf, bal_rf, bal_rd = {}, {}, {}
    kom_name_by_code = {}
    kom_lan = {}
    rf_name_by_code = {}   # länkod(2) -> regionnamn (RF valområdesnamn)
    for grp in groups.values():
        pis = grp_indices(grp)
        if not pis: continue
        fk = grp["fk"] or "Övriga"
        if grp["vt"] == 4:  # KF
            first = persons[grp["mem"][0][0]]
            kod = first.get("kf", "")
            if not kod: continue
            kom_name_by_code[kod] = code_to_name.get(kod, first.get("ort", "") or kod)
            kom_lan[kod] = LAN_CODE_TO_IDX.get(kod[:2], LAN_OKAND)
            bal_kf.setdefault(kod, {}).setdefault(fk, []).extend(pis)
        elif grp["vt"] == 2:  # RF (region) - nyckel = valområdeskod (länskod), namn = regionnamn
            key = grp.get("omr_kod2") or ""
            nm = grp.get("omr_namn") or grp["area"] or grp["lbl"]
            if not key: continue
            rf_name_by_code[key] = nm
            bal_rf.setdefault(key, {}).setdefault(fk, []).extend(pis)
        elif grp["vt"] == 1:  # RD (riksdag) - nyckel = valkretsnamn
            key = grp["area"] or grp["lbl"]
            bal_rd.setdefault(key, {}).setdefault(fk, []).extend(pis)

    # län -> kommunlista
    bal_lan = {}
    for kod, li in kom_lan.items():
        bal_lan.setdefault(str(li), []).append([kod, kom_name_by_code.get(kod, kod)])
    for li in bal_lan:
        bal_lan[li].sort(key=lambda x: x[1])

    # kommun -> (regionkod, rd-valkrets, namn, länidx)
    kom_map = {}
    for kod in bal_kf:
        li = kom_lan.get(kod, LAN_OKAND)
        lankod = kod[:2]
        rfkey = lankod if lankod in bal_rf else None   # Gotland saknar regionval
        kom_map[kod] = [rfkey, geo_rvk.get(kod, ""), kom_name_by_code.get(kod, kod), li]

    # Region- och riksdagsballotar byggs ur flera valkretsar (samma kandidater upprepas
    # per valkrets) -> avduplicera per parti, behåll listordning.
    def _dedup(store):
        for key, parts in store.items():
            for fk, lst in parts.items():
                seen, out = set(), []
                for pi in lst:
                    if pi in seen: continue
                    seen.add(pi); out.append(pi)
                parts[fk] = out
    _dedup(bal_rf); _dedup(bal_rd)

    ballot = {"lan": bal_lan, "kf": bal_kf, "rf": bal_rf, "rd": bal_rd,
              "komMap": kom_map, "rfNames": rf_name_by_code}
...
def main():
    ...
    ballot = data.pop("_ballot", {})

    # Injicera BALLOT-blocket (bläddra län -> kommun -> kandidater)
    m_bal = re.search(r'(<script id="BALLOT" type="application/json">).*?(</script>)', html, re.S)
    if m_bal:
        html = html[:m_bal.start()] + m_bal.group(1) + \
               json.dumps(ballot, ensure_ascii=False, separators=(",", ":")) + \
               m_bal.group(2) + html[m_bal.end():]
        _, data_span = read_block(html, "DATA")
    else:
        print("VARNING: <script id=\"BALLOT\"> saknas i HTML - bläddringen hoppas över.")
